...
<style>
  ...
  .rounded-custom-ulasan{
  border-radius: 3px 70px 3px 21px;
  }
</style>
  <div class="container">
    <div class="row py-5 text-center"> 
      <div class="col-lg-3 mx-auto m-2">
        <a href="#ulasan" class="btn btn-outline-secondary w-100 h-100"><i class="fa fa-sticky-note" aria-hidden="true"></i> {{$ulasan->where('buku_id', $buku->id)->count()}} Ulasan</a>
      </div>
      @if (is_null($favorit))
      <div class="col-lg-3 mx-auto m-2">
      ...
      <div class="row py-4" id="ulasan">
        <div class="col-md-12 col-lg-12">
          @foreach ($ulasan->where('buku_id', $buku->id) as $item)
            ...
            <div class="p-3 rounded-custom-ulasan shadow mb-5" style="background-color: #a7c5eb">
              <p class="h6 text-bold">{{$item->user->name}}</p>
              <hr>
              <p class="text-bold">{{$item->ulasan}}</p>
            </div>
          @endforeach
